...
def _delete_old_files(path, keep_model=False, model_id=""):
    ...
    files = [
        os.path.join(path, f) for f in os.listdir(path) if f.startswith("model.ckpt-")
    ]
...
class ExamplesPerSecondHook(session_run_hook.SessionRunHook):
    ...
    def __init__(self, batch_size, every_n_steps=100, every_n_secs=None):
        ...
        if (every_n_steps is None) == (every_n_secs is None):
            raise ValueError(
                "exactly 1 of every_n_steps and every_n_secs should be provided."
            )
    ...
    def after_run(self, run_context, run_values):
        ...
        if self._timer.should_trigger_for_step(global_step):
            ...
            if elapsed_time is not None:
                ...
                tf_logging.info(
                    f"Average examples/sec: {average_examples_per_sec} "
...
class TimeOutHook(session_run_hook.SessionRunHook):
    ...
    def __init__(self, timeout_sec, t0, every_n_steps=100):
        ...
        self._timer = basic_session_run_hooks.SecondOrStepTimer(
            every_steps=every_n_steps, every_secs=None
        )
        self._global_step_tensor = None
